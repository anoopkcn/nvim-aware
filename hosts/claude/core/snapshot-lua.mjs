// GENERATED FILE — do not edit. Source: core/snapshot-lua.mjs. Regenerate with `npm run build` (or `node scripts/build.mjs`).
/**
 * The Lua evaluated inside Neovim to produce a snapshot.
 *
 * Isolated here so it can be read as Lua rather than as a string buried in
 * fetching logic. It encodes every selection / quickfix / buffer edge case and
 * must not be rewritten casually — the mode handling in `read_selection` in
 * particular is load-bearing (charwise vs linewise vs blockwise, active vs
 * last selection).
 *
 * Note on empty collections: Neovim encodes an empty Lua table as an empty JSON
 * array, so surroundingLines / buffers / windows arrive as arrays even when
 * empty. toSnapshot normalizes defensively anyway.
 *
 * Editing note: this Lua lives inside a JS template literal, so a backtick or a
 * dollar-brace in the Lua (including in comments) will terminate or interpolate
 * the string and break the build. Keep both out.
 */

/** Lightweight server summary: cwd, current file, cursor. Used to probe candidates. */
export const SUMMARY_LUA = String.raw`
(function()
  local api = vim.api
  local fn = vim.fn
