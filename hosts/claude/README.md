...
No Neovim plugin or UI changes are required. The integration reads editor state over
`nvim --server <addr> --remote-expr`.

This plugin is one host of the [nvim-aware](https://github.com/anoopkcn/nvim-aware)
monorepo; the shared implementation lives in the repo's `core/` and is vendored into
this directory's `core/` by the build (do not edit those files here).
...
```bash
claude --plugin-dir /path/to/nvim-aware/hosts/claude
```
